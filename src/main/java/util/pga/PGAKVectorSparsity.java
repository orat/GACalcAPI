 ...
 */
public class PGAKVectorSparsity extends CGAMultivectorSparsity {

    private static CGACayleyTable cgaCayleyTable = CGACayleyTableGeometricProduct.instance();

    public static Map<Integer, PGAKVectorSparsity> map = new HashMap<>();
    private int grade = -1;

    public static PGAKVectorSparsity instance(int grade) {
        PGAKVectorSparsity result = map.get(grade);
        if (result == null) {
            result = new PGAKVectorSparsity(grade);
     ...
     */
    public static PGAKVectorSparsity createBasisBladeSparsity(int index) {
        ...
        return new PGAKVectorSparsity(new int[]{index});
    }
     ...
     */
    PGAKVectorSparsity(int[] row) {
        super(row);
        grade = getGrade(row);
        if (grade < 0) {
            throw new IllegalArgumentException("The given indizes correspond to different grades!");
        }
    }
    ...
    /**
     * Count of blades for the given grade - corresponding to the second value of the accumulated column
     * indizes.
     *
     * Das ist CGA spezifisch - müsste es aber nicht sein. Aus der Zahl der basisBladeNames könnte die Zahl
     * der basisvektoren bestimmt werden und daraus die Zahl der Basisvektoren eines bestimmten Grades. TODO
     *
     * @param grade
    ...
    private static int[] rows(String[] basisBladeNames, int grade, int colind) {
        ...
        for (int i = 0; i < basisBladeNames.length; i++) {
            if (basisBladeNames[i].length() == charcount) {
                rows[j++] = i;
            }
        }
        return rows;
